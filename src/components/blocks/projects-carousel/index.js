...
const BlockProjectsCarousel = ({ data }) => {
  ...
  const slideCount = carouselData.manualSelection ? carouselData.projects.length : carouselData.projectsMax

  return (
    <section
      className={`flex flex-col mt-36 mb-20 overflow-x-hidden pl-5 sm:mt-44 sm:pl-12 lg:pl-20 ${slideCount < 4 ? 'xl:ml-our_latest_work' : ''
        }`}
    >
      ...
      </div>
      {
        (carouselData.manualSelection && carouselData.projects.length) && (
          <ProjectsCarousel slides={carouselData.projects.map(project => {
            return {
              image: project.image
                ...
                : project.project.nodes[0].featuredImage?.node
                  ? project.project.nodes[0].featuredImage.node
                  : null,
              title: project.title ? project.title : project.project.nodes[0].title,
        ...
        !carouselData.manualSelection && (
          <StaticQuery
            ...
            render={(data) => {
              ...
              return (
                <ProjectsCarousel slides={data.allWpProject.edges.map(project => {
                  return {
                    image: project.node.featuredImage ? project.node.featuredImage.node : null,
